...
@section('content')
    ...
    <div>
        <form method="GET" action="{{ route('emprunts.index') }}">
            ...
            <input type="date" name="date_retour" value="{{ request('date_retour') }}">
            <button type="submit">Rechercher</button>
            <a href="{{ route('emprunts.index') }}">Réinitialiser</a>
        </form>
    ...
    <table style="border-collapse: collapse; width: 100%;" border="1" cellpadding="5">
        <thead>
            <tr>
                ...
                <th style="border: 1px solid black;">Date retour</th>
                <th style="border: 1px solid black;">Lecteur</th>
                <th style="border: 1px solid black;">Livres empruntés</th>
        ...
        <tbody>
            @forelse($emprunts as $emprunt)
                <tr>
                    <td style="border: 1px solid black;">
                        {{ $loop->iteration + ($emprunts->currentPage() - 1) * $emprunts->perPage() }}
                    </td>
                    <td style="border: 1px solid black;">{{ $emprunt->date_debut }}</td>
                    <td style="border: 1px solid black;">{{ $emprunt->date_retour }}</td>
                    <td style="border: 1px solid black;">
                        {{ optional($emprunt->lecteur?->user)->name ?? '-' }}
                    </td>
                    <td style="border: 1px solid black;">
            ...
            @empty
                <tr>
                    <td colspan="6" style="text-align: center; border: 1px solid black;">Aucun emprunt trouvé.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
